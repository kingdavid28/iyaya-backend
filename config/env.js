...
const validateEnv = () => {
  ...
  if (usingSupabase) {
    ...
    console.error(
      "- Either SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY for Supabase",
    );
  ...
  if (process.env.JWT_SECRET?.length < 32) {
    console.error(
      chalk.red.bold("JWT_SECRET must be at least 32 characters long"),
    );
    process.exit(1);
  }
  ...
  if (
    ...
    !/^(\*|https?:\/\/[^,\s]+(,\s*https?:\/\/[^,\s]+)*)$/.test(
      process.env.CORS_ORIGIN,
    )
  ) {
    console.error(
      chalk.red.bold(
        'CORS_ORIGIN must be either "*" or a comma-separated list of valid URLs',
      ),
    );
    process.exit(1);
  }
...
module.exports = {
  ...
  database: {
    // Support both Supabase and MongoDB (legacy)
  ...
  jwt: {
    ...
    refreshSecret:
      process.env.JWT_REFRESH_SECRET || `${process.env.JWT_SECRET}_FALLBACK`,
  ...
  cors: {
    origin: (origin, callback) => {
      ...
      if (
        ...
        allowedOrigins.some(
          (o) =>
            origin === o || new URL(origin).hostname === new URL(o).hostname,
        )
      ) {
