...
const connectWithRetry = async (retries = 5, delay = 1000) => {
  try {
    ...
    const conn = await mongoose.connect(
      process.env.MONGODB_URI || "mongodb://localhost:27017/iyaya",
      {
        ...
        serverSelectionTimeoutMS:
          parseInt(process.env.MONGO_CONNECTION_TIMEOUT) || 30000,
    ...
    if (retries === 0) {
      console.error(
        chalk.red.bold("❌ Max retries reached. Failed to connect to MongoDB:"),
        err.message,
      );
      process.exit(1);
    }
    console.warn(
      chalk.yellow.bold(
        `⚠️ MongoDB connection failed, retrying in ${delay}ms... (${retries} retries left)`,
      ),
    );
...
const connectDB = async () => {
  try {
    ...
    console.error(
      chalk.red.bold("❌ Failed to connect to MongoDB after retries:"),
      err.message,
    );
    process.exit(1);
  }
