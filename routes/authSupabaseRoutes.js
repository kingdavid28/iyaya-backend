 ...
 * Updated to use Supabase auth controller
 */
...
const {
  ...
  checkEmailExists,
} = require("../controllers/authSupabase");
...
const loadController = (name) => {
  try {
    ...
    if (name === "authSupabase") {
      console.error(
        "🚨 Critical: Supabase Auth controller is required for the application to function",
      );
      ...
      console.error("2. It has the correct name (authSupabase.js)");
...
const authController = loadController("authSupabase") || {
  register: (req, res) =>
...
const verifyMethods = (controller, methods, controllerName) => {
  ...
  methods.forEach((method) => {
    if (typeof controller[method] !== "function") {
      ...
      } else if (controllerName === "authSupabase") {
...
router.get("/health-check", (req, res) => {
  const healthCheck = {
    ...
    dbStatus: "supabase_connected", // Updated for Supabase
...
router.get("/docs", (req, res) => {
  const apiDocumentation = {
    name: "Supabase Authentication Service API",
    version: "1.0.0",
    description: "Authentication API using Supabase for user management",
    endpoints: [
      {
        method: "POST",
        path: "/register",
