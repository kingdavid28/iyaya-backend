...
const resolveMongoId = (user) => {
  ...
  console.log("❌ Could not resolve MongoDB ID from:", {
...
exports.getCaregiverProfile = async (req, res) => {
  try {
    ...
    if (!userMongoId) {
      return res
        .status(401)
        .json({ success: false, error: "User mapping not found" });
    }
...
exports.searchCaregivers = async (req, res) => {
  try {
    ...
    console.log("🔍 Caregiver search request:", {
      skills,
      minRate,
      maxRate,
      daysAvailable,
    ...
    const caregiverProfiles = await Caregiver.find({ userId: { $in: userIds } })
      .select(
        "userId name skills experience hourlyRate availability rating ageCareRanges profileImage address location bio hasCompletedJobs",
      )
      .lean();
    ...
    console.log("📊 Search results:", {
      count,
      caregivers: filteredCaregivers.length,
    });
    console.log(
      "👥 Found caregivers:",
    ...
    await logActivity("PROVIDER_SEARCH", {
...
exports.getCaregiverDetails = async (req, res) => {
  try {
    ...
    await logActivity("PROVIDER_VIEW", {
    ...
    await logActivity("PROVIDER_VIEW_ERROR", {
      caregiverId: req.params.id,
      error: err.message,
    });
    res
...
exports.updateCaregiverProfile = async (req, res) => {
  try {
    ...
    if (Array.isArray(transformedCertifications))
      updateData.certifications = transformedCertifications;
    ...
    if (!caregiver) {
      ...
      console.log(
        "✅ New caregiver profile created successfully:",
        hydrated._id,
      );
      return res.json({
        success: true,
    ...
    const verifyUpdate = await Caregiver.findById(caregiver._id).select(
      "name bio skills hourlyRate updatedAt",
    );
      ...
      try {
        ...
        if (freshCaregiver) {
          const completionPercentage =
            freshCaregiver.profileCompletionPercentage;
          if (
            completionPercentage >= 80 &&
            !freshCaregiver.verification.profileComplete
          ) {
...
exports.uploadPortfolioImages = async (req, res) => {
  try {
    ...
    if (!caregiver) {
      return res
        .status(404)
        .json({ success: false, error: "Caregiver profile not found" });
    }
...
exports.getVerificationStatus = async (req, res) => {
  try {
    ...
    const caregiver = await Caregiver.findOne({ userId: userMongoId }).select(
      "verification profileCompletionPercentage",
    );
